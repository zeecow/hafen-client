...
public class BAttrWnd extends Widget {
	...
	Widget prev;
	prev = add(CharWnd.settip(new Img(attrf.render("Base Attributes").tex()), "gfx/hud/chr/tips/base"), Coord.z);
	attrs = new ArrayList<>();
	Attr aw;
	...
	prev = add(CharWnd.settip(new Img(attrf.render("Food Event Points").tex()), "gfx/hud/chr/tips/fep"), prev.pos("bl").x(0).adds(0, 10));
	feps = add(new FoodMeter(), prev.pos("bl").adds(5, 2));

	int ah = attrs.get(attrs.size() - 1).pos("bl").y - attrs.get(0).pos("ul").y;
	prev = add(CharWnd.settip(new Img(attrf.render("Food Satiations").tex()), "gfx/hud/chr/tips/constip"), width, 0);
	...
	prev = add(CharWnd.settip(new Img(attrf.render("Hunger Level").tex()), "gfx/hud/chr/tips/hunger"), prev.pos("bl").x(width).adds(0, 10));
