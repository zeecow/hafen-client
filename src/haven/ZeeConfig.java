...
public class ZeeConfig {
    ...
    static final String POSE_PLAYER_CAULDRON_STIR = "gfx/borka/stirran";
    ...
    static Message getDrawableData(Gob gob) {
    ...
    static void windowApplySavedPosition(Window window, String windowTitle) {
        Coord c;
        ...
        new ZeeThread(){
            public void run() {
                try {
                    ...
                    ZeeManagerMinimap.toggleMinimapOptionsButton(gameUI.mapfile, isMinimapCompacted());
    ...
    public static void toggleMineSupports() {
        ...
        // no radius, cover = add radius
            ...
            List<Gob> sups = findGobsByNameEndsWith("/minebeam", "/column", "/minesupport",
                    "/naturalminesupport", "/towercap", "/ladder","/monumentalcolumn",
                    "/stonearchtunnel","/reinforcedtunnel","/timbertunnel");
            if (!sups.isEmpty()) {
                for (Gob sup : sups) {
    ...
    static void toggleCheeserack(Gob cheeserack){
        if (highlightCheeserack){
            ...
            if (trays==0)
                return;
            if (trays==3)
                ZeeConfig.addGobColor(cheeserack, ZeeGobColor.COLOR_DARK_GREEN);
            else
    ...
    static void toggleGardenpot(Gob pot){
        if (highlightGardenpot){
            ...
            if (plants < 2)
                return;
            ZeeConfig.addGobColor(pot, ZeeGobColor.COLOR_DARK_GREEN);
        }else {
    ...
    public static boolean checkKeyPressed(KeyEvent ev) {
        ...
        // fast zoom
            ...
            return gameUI.map.camera.wheel(new Widget.MouseWheelEvent(Coord.z,1,10));
    ...
    public static void clickOpenBelt() {
        ...
        if (belt != null) {
            belt.mousedown(new Widget.MouseDownEvent(Coord.z, 3));
            ZeeManagerItems.invBelt = null;
        }
    }
    ...
    static void gameUIResized() {
        ...
        windows.forEach( w -> {
            ...
            if (w.c.x >= mid.x  &&  change.x!=0){
                ...
                if(w.cap.contentEquals("Map") && ZeeManagerMinimap.mapWndLastPos!=null)
                    ZeeManagerMinimap.mapWndLastPos.x += change.x;
                // save pos
                saveWindowPos(w);
            }
        });
    ...
    static boolean isMinimapCompacted(){
        return ZeeManagerMinimap.isMinimapCompacted();
    }
